...
public class RemoveItemFromCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
           ...
           if(session != null){
            String[] removeItem= request.getParameterValues("chkItem");
               if(removeItem != null){
                   ProductDAO dao = (ProductDAO) session.getAttribute("LIST_ITEM_IN_CART");
                   for(String item: removeItem){
                       dao.removeItems(item);
                   }
                   session.setAttribute("LIST_ITEM_IN_CART", dao);
               }
               
           }
        } catch (SQLException ex) {
            log("RemoveItemFromCartServlet SQLException"+ ex.getMessage());
        } catch (NamingException ex) {
            log("RemoveItemFromCartServlet NamingException"+ ex.getMessage());

        } catch (ClassNotFoundException ex) {
           log("RemoveItemFromCartServlet ClassNotFoundException"+ ex.getMessage()); 
        } finally{
            String url = roadmap.get("viewcartJSP");
